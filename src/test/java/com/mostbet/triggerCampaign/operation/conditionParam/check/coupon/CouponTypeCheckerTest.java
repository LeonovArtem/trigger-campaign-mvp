...
public class CouponTypeCheckerTest extends BaseFunctionalTest {
    @Autowired
    private EventProcessService eventProcessService;

    @Test
    @DataSet(
            value = {"/dataSet/operation/conditionParam/check/coupon/type/dataset.json"},
            executeScriptsBefore = "truncate_all.sql",
            executeScriptsAfter = "truncate_all.sql"
    )
    public void whenConditionTypeEqualsCouponType_thenCampaignIsFulfilled() {
        List<CampaignProcessResponse> responses = eventProcessService.process(
                assembleRequest(CouponType.ORDINAR)
        );
    ...
    public void whenConditionTypeIsNotEqualsCouponType_thenCampaignIsNotFulfilled() {
        List<CampaignProcessResponse> responses = eventProcessService.process(
                assembleRequest(CouponType.EXPRESS)
        );
    ...
    private static RequestDto<RequestDto.Payload> assembleRequest(CouponType couponType) {
        CouponCloseRequestPayloadDto couponCloseRequestPayloadDto = new CouponCloseRequestPayloadDto();
        couponCloseRequestPayloadDto.setCouponType(couponType);
